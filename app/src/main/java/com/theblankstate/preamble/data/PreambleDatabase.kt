...
abstract class PreambleDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        private val MIGRATION_27_28 = object : Migration(27, 28) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE `tasks` ADD COLUMN `assignedByUid` TEXT DEFAULT NULL")
                db.execSQL("ALTER TABLE `tasks` ADD COLUMN `assignedByName` TEXT DEFAULT NULL")
        ...
        val MIGRATION_31_32 = object : Migration(31, 32) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE `focus_sessions` ADD COLUMN `durationSelectedSeconds` INTEGER NOT NULL DEFAULT 1500")
                db.execSQL("ALTER TABLE `focus_sessions` ADD COLUMN `actualDurationCompletedSeconds` INTEGER NOT NULL DEFAULT 1500")
                db.execSQL("ALTER TABLE `focus_sessions` ADD COLUMN `completionStatus` TEXT NOT NULL DEFAULT 'COMPLETED'")
                db.execSQL("ALTER TABLE `focus_sessions` ADD COLUMN `createdTimestamp` INTEGER NOT NULL DEFAULT 0")
            }
        }
    }
}
