...
interface FocusSessionDao {
    ...
    @Query("SELECT COALESCE(SUM(durationSeconds), 0) FROM focus_sessions WHERE date = :date")
    suspend fun getTotalFocusSecondsForDate(date: String): Int

    // Date range queries (for weekly/monthly charts)
    @Query("SELECT date, COUNT(*) as sessionCount, SUM(durationSeconds) as totalSeconds FROM focus_sessions WHERE date IN (:dates) GROUP BY date")
    ...
    // Best focus day
    @Query("SELECT date, SUM(durationSeconds) as totalSeconds FROM focus_sessions GROUP BY date ORDER BY totalSeconds DESC LIMIT 1")
    suspend fun getBestFocusDay(): BestFocusDay?

    // Top focused tasks
    @Query("SELECT taskId, taskTitle, COUNT(*) as sessionCount, SUM(durationSeconds) as totalSeconds FROM focus_sessions WHERE taskId IS NOT NULL GROUP BY taskId ORDER BY totalSeconds DESC LIMIT :limit")
    ...
    @Query("SELECT COALESCE(CAST(SUM(durationSeconds) AS REAL) / MAX(COUNT(DISTINCT date), 1), 0) FROM focus_sessions")
