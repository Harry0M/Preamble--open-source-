...
object AiPromptFactory {
    ...
    fun buildSystemPrompt(existingTasks: List<Task>? = null, smartBreakdown: Boolean = false, isNotificationEdit: Boolean = false): String {
        ...
        sb.appendLine("  3. PERSISTENCE INTENT (STRONG SIGNAL → rollover=true): user phrases the task as continuing until a condition is met. Patterns: 'jab tak X na ho/mile/aaye', 'jab tak X nahi hoga tab tak', 'X milne tak', 'X hone tak', 'until I find/get/finish', 'keep doing until', 'har roz try karna jab tak', 'tab tak karta rahunga'. If matched → rollover=true (overrides time-window detection below).")
        sb.appendLine("  4. If 'deadline_time' is set → rollover=false (time-bound, loses meaning next day).")
        sb.appendLine("  5. If user mentions a time-of-day window for TODAY (aaj shaam, aaj raat, aaj dopahar, tonight, this evening, this morning, subah, shaam, raat, dopahar) → rollover=false.")
        sb.appendLine("  6. If task is a specific time-bound EVENT happening today (dinner, show, movie, party, concert, visit someone, going somewhere, attending, meeting, scheduled call) → rollover=false.")
        sb.appendLine("  7. Otherwise (today + open-ended work/chore/errand/creative) → rollover=true. This includes: build/create/make/design/write/code/finish/prepare/fix/call someone back/buy/research/read/learn/plan. NO time mentioned = rollover.")
        sb.appendLine("Examples:")
