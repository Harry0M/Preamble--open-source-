...
// ─── AI V2 MODEL REGISTRY ROUTES ───

const V2_CONFIG_DOC = 'v2_config/models';

// Get all AI V2 models
app.get('/api/v2/models', requireAuth, async (req, res) => {
  try {
    const doc = await firestoreDb.doc(V2_CONFIG_DOC).get();
    const data = doc.exists ? doc.data() : {};
    res.json({ models: data.models || [] });
  } catch (err) {
    console.error('Error fetching V2 models:', err);
    res.status(500).json({ error: 'Failed to fetch AI V2 models' });
  }
});

// Save (overwrite) the full models array
app.put('/api/v2/models', requireAuth, async (req, res) => {
  try {
    ...
    if (models.length > 50) {
      return res.status(400).json({ error: 'Maximum 50 model entries allowed' });
    }

    // Validate each model entry
    for (const m of models) {
      if (!m.provider || m.provider.length < 1 || m.provider.length > 100) {
        return res.status(400).json({ error: `Invalid provider name: must be 1-100 chars` });
      }
      if (!m.modelId || m.modelId.length < 1 || m.modelId.length > 200) {
        return res.status(400).json({ error: `Invalid model ID: must be 1-200 chars` });
      }
      if (!m.displayName || m.displayName.length < 1 || m.displayName.length > 60) {
        return res.status(400).json({ error: `Invalid display name: must be 1-60 chars` });
      }
      if (typeof m.costPerMillionTokens !== 'number' || m.costPerMillionTokens <= 0) {
        return res.status(400).json({ error: `Cost per million tokens must be a positive number` });
      }
    }

    // Ensure exactly one default model among enabled models
    const enabledModels = models.filter(m => m.enabled);
    const defaults = models.filter(m => m.isDefault);
    if (defaults.length !== 1) {
      return res.status(400).json({ error: 'Exactly one model must be set as default' });
    }
    if (!defaults[0].enabled) {
      return res.status(400).json({ error: 'The default model must be enabled' });
    }

    await firestoreDb.doc(V2_CONFIG_DOC).set({ models }, { merge: true });
    res.json({ success: true, models });
  } catch (err) {
    console.error('Error saving V2 models:', err);
    res.status(500).json({ error: 'Failed to save AI V2 models' });
  }
});

// ─── AI V2 TIER BUDGET ROUTES ───

// Get tier budgets and model cost rates
app.get('/api/v2/tier-budgets', requireAuth, async (req, res) => {
  try {
    const doc = await firestoreDb.doc(V2_CONFIG_DOC).get();
    const data = doc.exists ? doc.data() : {};
    const tierBudgets = data.tierBudgets || {
      pro_student: 50000,
      pro_youth: 100000,
      pro_standard: 200000
    };
    ...
    res.json({ tierBudgets, models });
  } catch (err) {
    console.error('Error fetching V2 tier budgets:', err);
    res.status(500).json({ error: 'Failed to fetch tier budgets' });
  }
});

// Update tier budgets
app.put('/api/v2/tier-budgets', requireAuth, async (req, res) => {
  try {
    const { tierBudgets } = req.body;
    if (!tierBudgets || typeof tierBudgets !== 'object') {
      return res.status(400).json({ error: 'tierBudgets object is required' });
    }

    const validTiers = ['pro_student', 'pro_youth', 'pro_standard'];
    for (const tier of validTiers) {
      if (tierBudgets[tier] === undefined) {
        return res.status(400).json({ error: `Missing budget for tier: ${tier}` });
      }
      const val = Number(tierBudgets[tier]);
      if (!Number.isFinite(val) || val <= 0) {
        return res.status(400).json({ error: `Budget for ${tier} must be a positive number` });
      }
    }

    const sanitized = {
      pro_student: Number(tierBudgets.pro_student),
      pro_youth: Number(tierBudgets.pro_youth),
      pro_standard: Number(tierBudgets.pro_standard)
    };

    await firestoreDb.doc(V2_CONFIG_DOC).set({ tierBudgets: sanitized }, { merge: true });
    res.json({ success: true, tierBudgets: sanitized });
  } catch (err) {
    console.error('Error saving V2 tier budgets:', err);
    res.status(500).json({ error: 'Failed to save tier budgets' });
  }
});
